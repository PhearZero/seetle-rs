use crate::{Algorithm, Bindings, KeyUsage, KeyOrIdentifier, SeetleError, CryptoKey, Seetle, SecureStorage, HardwareBound};
...
impl NordicBackend {
    ...
    async fn get_metadata(&self, identifier: &str) -> Result<NordicMetadata, SeetleError> {
    ...
    fn check_usage(&self, metadata: &NordicMetadata, usage: KeyUsage) -> Result<(), SeetleError> {
...
unsafe extern "C" {
    ...
    fn psa_export_key(key_id: PsaKeyId, data: *mut u8, data_size: usize, data_length: *mut usize) -> PsaStatus;
}
...
const PSA_ALG_EDDSA: PsaAlgorithm = 0x06000700;
...
const PSA_KEY_TYPE_ECC_KEY_PAIR_ED25519: PsaKeyType = 0x71004d;
const PSA_KEY_TYPE_RSA_KEY_PAIR: PsaKeyType = 0x7001;
const PSA_KEY_TYPE_AES: PsaKeyType = 0x2400;
const PSA_KEY_TYPE_RAW_DATA: PsaKeyType = 0x10010000;
...
fn map_algorithm(alg: &Algorithm) -> PsaAlgorithm {
    match alg {
        ...
        Algorithm::Ed25519 { .. } => PSA_ALG_EDDSA,
...
fn map_key_type(alg: &Algorithm) -> PsaKeyType {
    match alg {
        ...
        Algorithm::Ed25519 { .. } => PSA_KEY_TYPE_ECC_KEY_PAIR_ED25519,
        Algorithm::RsaPss { .. } => PSA_KEY_TYPE_RSA_KEY_PAIR,
        Algorithm::AesGcm { .. } => PSA_KEY_TYPE_AES,
        Algorithm::Raw { .. } => PSA_KEY_TYPE_RAW_DATA,
        _ => 0,
    }
}
    ...
    async fn generate_key(
        ...
        if let Some(mut b) = bindings {
            ...
            unsafe {
                ...
                if b.hardware_bound == HardwareBound::Yes {
        ...
        match key {
            KeyOrIdentifier::Identifier(id) => {
                ...
                let signature_size = match &metadata.algorithm {
                    Algorithm::RsaPss { modulus_length, .. } => (*modulus_length / 8) as usize,
                    _ => 64, // Enough for P-256 and Ed25519
                };
    ...
    async fn export_key(
        ...
        match key {
            KeyOrIdentifier::Identifier(id) => {
                ...
                let mut data_len = 0;
                let status = unsafe { psa_export_key(metadata.psa_id, data.as_mut_ptr(), data.len(), &mut data_len) };
                if status != PSA_SUCCESS {
                    return Err(map_psa_error(status));
                }
...
mod nordic_tests {
    ...
    #[tokio::test]
    async fn test_nordic_ed25519_metadata() {
        let storage: Arc<dyn SecureStorage> = Arc::new(MemoryStorage::new());
        let backend = NordicBackend::new(storage.clone());
        
        let bindings = Bindings {
            identifier: "test-ed25519".into(),
            hardware_bound: HardwareBound::Yes,
            ..Default::default()
        };
        let metadata = NordicMetadata {
        ...
        let metadata_bytes = serde_json::to_vec(&metadata).unwrap();
        storage.set_item("test-ed25519", metadata_bytes).await.unwrap();
        
        let retrieved_metadata = backend.get_metadata("test-ed25519").await.unwrap();
        assert!(matches!(retrieved_metadata.algorithm, Algorithm::Ed25519 { .. }));
        assert_eq!(retrieved_metadata.psa_id, 456);
        
        // Check mapping functions
        assert_eq!(map_algorithm(&retrieved_metadata.algorithm), PSA_ALG_EDDSA);
        assert_eq!(map_key_type(&retrieved_metadata.algorithm), PSA_KEY_TYPE_ECC_KEY_PAIR_ED25519);
        assert_eq!(get_key_bits(&retrieved_metadata.algorithm), 255);
    }
}
