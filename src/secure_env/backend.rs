...
impl SecureEnvBackend {
    ...
    async fn get_metadata(&self, identifier: &str) -> Result<SecureEnvMetadata, SeetleError> {
    ...
    #[cfg(any(target_os = "android", target_os = "ios"))]
    fn check_usage(&self, metadata: &SecureEnvMetadata, usage: KeyUsage) -> Result<(), SeetleError> {
...
impl Seetle for SecureEnvBackend {
    async fn generate_key(
        ...
        #[cfg(any(target_os = "android", target_os = "ios"))]
        {
            if let Some(mut b) = bindings {
                ...
                let metadata = SecureEnvMetadata {
                    bindings: b.clone(),
                    algorithm,
                    usages: key_usages,
                };
    ...
    async fn export_key(&self, _format: String, key: KeyOrIdentifier) -> Result<Vec<u8>, SeetleError> {
        ...
        #[cfg(any(target_os = "android", target_os = "ios"))]
        {
            ...
            // In Android/iOS, exporting a private key from Keystore/SecureEnclave is usually NOT possible
            // unless it's a symmetric key and created with export allowed.
            // For ECDSA, usually only the public key can be exported.
            Err(SeetleError::NotSupported)
        }
    }
